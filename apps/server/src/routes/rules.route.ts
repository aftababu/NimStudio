...
rulesRoute.post('/', async (c) => {
  try {
    ...
    const allRules = await db.select({ priority: globalRules.priority }).from(globalRules);
    const minPriority = allRules.length > 0 ? Math.min(...allRules.map(r => r.priority)) : 0;
...
rulesRoute.patch('/:id', async (c) => {
  try {
    ...
    const updates = await c.req.json<Partial<typeof globalRules.$inferSelect>>();
