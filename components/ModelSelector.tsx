...
export function ModelSelector() {
  const { activeModelId, setActiveModelId, activeConversationId } =
    useChatStore();
  ...
  const selectMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(
        `http://localhost:3001/api/models/${encodeURIComponent(id)}/select`,
        { method: "PUT" },
      );
  ...
  const removeMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(
        `http://localhost:3001/api/models/${encodeURIComponent(id)}`,
        { method: "DELETE" },
      );
  ...
  const renameMutation = useMutation({
    mutationFn: async ({ id, name }: { id: string; name: string }) => {
      const res = await fetch(
        `http://localhost:3001/api/models/${encodeURIComponent(id)}/rename`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleSelectChange = async (id: string) => {
    ...
    if (activeConversationId) {
      try {
        await fetch(
          `http://localhost:3001/api/chat/conversations/${activeConversationId}/preferences`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-xs px-sm py-xs border border-outline-variant rounded-DEFAULT bg-surface">
        ...
        <span className="font-body-md text-body-md text-on-surface-variant animate-pulse w-24 inline-block">
          Loading...
        </span>
      </div>
    );
            ...
            {models.map((model) => (
              ...
              >
                <div className="flex flex-col min-w-0 pr-8">
                  <span
                    className={`text-[15px] truncate ${activeModelId === model.id ? "text-primary font-medium" : "text-on-surface"}`}
                  >
                    {model.name}
                  </span>
                ...
                {activeMenuId === model.id && (
                  <div
                    className="absolute right-8 top-8 w-32 bg-surface-container-high border border-outline-variant rounded-md shadow-lg z-[60] py-1"
        ...
        <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center backdrop-blur-sm">
          <div className="bg-surface border border-outline-variant rounded-xl shadow-2xl w-[400px] p-6 animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-sm text-on-surface-variant mb-4">
              Enter the exact Model ID from NVIDIA NIM (e.g.{" "}
              <code>meta/llama-3.2-1b-instruct</code>).
            </p>
            <input
              type="text"
              value={newModelId}
